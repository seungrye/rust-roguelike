...
impl<'a> System<'a> for MeleeCombatSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for (_entity, want_to_melee, name, stats) in
            (&entities, &wants_to_melee, &names, &combat_stats).join()
        {
            if stats.hp > 0 {
                let target_stats = combat_stats.get(want_to_melee.target).unwrap();
                if target_stats.hp > 0 {
                    let target_name = names.get(want_to_melee.target).unwrap();
                    ...
                    if damage == 0 {
                        ...
                        SufferDamage::new_damage(&mut inflict_damage, want_to_melee.target, damage);
                    }
                }
            }
        }
